...
pub(crate) static JOBS: LazyLock<[Job; 22]> = LazyLock::new(|| {
    ...
    [
        ...
        Job::mkdir(p("old")),
        Job::move_file(p("ZedVela.exe"), p("old\\ZedVela.exe")),
        Job::mkdir(p("old\\bin")),
        ...
        // Copy new files
        Job::move_file(p("install\\ZedVela.exe"), p("ZedVela.exe")),
...
fn zed_launch_command(app_dir: &Path, launch_arguments: &[OsString]) -> std::process::Command {
    let mut command = std::process::Command::new(app_dir.join("ZedVela.exe"));
...
mod test {
    ...
    fn test_zed_launch_command_preserves_arguments() {
        ...
        let command = zed_launch_command(Path::new(r"C:\Program Files\Zed Vela"), &arguments);

        assert_eq!(
            command.get_program(),
            Path::new(r"C:\Program Files\Zed Vela\ZedVela.exe").as_os_str()
        );
        assert_eq!(
